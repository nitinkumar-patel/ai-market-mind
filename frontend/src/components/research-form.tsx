...
export function ResearchForm({ onSubmit, isLoading }: Props) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
      <h1 className="text-lg font-bold text-slate-900 mb-0.5">Market Research Agent</h1>
      ...
      <form onSubmit={handleSubmit}>
        <div className="flex gap-2.5 flex-wrap sm:flex-nowrap">
          <input
            ...
            autoFocus
            className="
              flex-1 min-w-0 px-3.5 py-2.5 text-sm rounded-lg border border-slate-200 outline-none
          ...
          <select
            ...
            disabled={isLoading}
            className="
              px-3 py-2.5 text-sm rounded-lg border border-slate-200 bg-white outline-none
              focus:border-blue-500 focus:ring-2 focus:ring-blue-500/10
            ...
            {isLoading ? (
              <>
                <svg className="animate-spin w-4 h-4 shrink-0" viewBox="0 0 24 24" fill="none">
                  ...
                  <path fill="white" fillOpacity=".75" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" />
                </svg>
                Researching…
              </>
            ) : (
      ...
      <div className="flex flex-wrap items-center gap-2 mt-3.5">
        ...
        {EXAMPLES.map((ex) => (
          <button
            ...
            disabled={isLoading}
            className="
              text-xs text-blue-600 bg-blue-50 hover:bg-blue-100 border border-blue-100
